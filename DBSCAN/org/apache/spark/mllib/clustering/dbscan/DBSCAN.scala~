...
class DBSCAN private (
  ...
  private def train(samples: RDD[(String, Vector)]): DBSCAN = {
	...
	/*minimumRectanglesWithCount.foreach{
	case (rectangle, count) =>
	logWarning(s"ORIGINAL: Min bounding rectangle of $count points : $rectangle")
	}*/
    ...
    val margins = samples.context.broadcast(localMargins)

    // assign each point to its proper partition
    val duplicated = for {
      point <- samples.map{ case (id, v) => DBSCANPoint(id, v)}
    ...
    val clusterIds = samples.context.broadcast(clusterIdToGlobalId)
  ...
  private def toMinimumBoundingRectangle(vector: Vector): DBSCANRectangle = {
	val point = DBSCANPoint("N/A", vector)
	val f = point.featuresA
	val min_corners = f.map(corner)
	val max_corners = min_corners.map(_ + minimumRectangleSize)
	DBSCANRectangle(min_corners, max_corners)
  }
