...
class LocalDBSCANNaive(eps: Double, minPoints: Int) extends Logging {
  ...
  def samplePoint = Array(new DBSCANLabeledPoint("N/A", Vectors.dense(Array(0D, 0D))))
  ...
  private def findNeighbors(
    ...
    all.view.filter{
	other => { sqrt(point.distanceSquared(other)) <= minDistance } //other distance functions can be used here
    }
